...
docker-build:
	...
	@echo "$(DOCKER) buildx build -f Dockerfile.cli.ubi9 -t $(IMAGE_PREFIX)argocd-cli:$(IMAGE_TAG) --build-arg TARGETARCH=$(TARGETARCH) --build-arg TARGETOS=$(TARGETOS) ."
	$(DOCKER) buildx build -f Dockerfile.cli.ubi9 -t $(IMAGE_PREFIX)argocd-cli:$(IMAGE_TAG) --build-arg TARGETARCH=$(TARGETARCH) --build-arg TARGETOS=$(TARGETOS) .
